...
public class VpnProfile implements Serializable, Cloneable {
    ...
    public String getOpenVPNConfigFile(Context context, boolean configForOvpn3) {
    ...
    public void writeConfigFile(Context context, String serverConfig,
            String serverIp, String protocol, String port, @Nullable String stunnelRoutingIp,
            @Nullable String X509Name) throws IOException {
        FileWriter cfg = new FileWriter(VPNLaunchHelper.getConfigFilePath(context));
        String serverConfigDecoded = null;
        //If server config is not null, get server config
        if (serverConfig != null) {
            serverConfigDecoded = new String(org.spongycastle.util.encoders.Base64.decode(serverConfig));
        }
        //If server config decoded is not null
        if (serverConfigDecoded != null) {
            String content = getConfigFile(context, serverIp, protocol, port, stunnelRoutingIp, X509Name, false,
                    serverConfigDecoded);
            cfg.write(content);
        }
    ...
    public void writeConfigFileBlocking(Context context, String serverConfig) throws IOException {
        FileWriter cfg = new FileWriter(VPNLaunchHelper.getConfigFilePath(context));
        String content = getConfigFileBlocking(context, serverConfig);
        cfg.write(content);
        cfg.flush();
        cfg.close();
    }

    private String getConfigFile(Context context, String serverIp, String protocol, String port,
        ...
        cfg.append("management-hold\n\n");
        if (X509Name != null) {
            cfg.append(String.format("verify-x509-name %s name \n", X509Name));
        }
        if (!configForOvpn3) {
            cfg.append(String.format("setenv IV_GUI_VER %s \n", openVpnEscape(getVersionEnvString(context))));
            String versionString = String
                    .format(Locale.US, "%d %s %s %s %s %s", Build.VERSION.SDK_INT, Build.VERSION.RELEASE,
                            NativeUtils.getNativeAPI(), Build.BRAND, Build.BOARD, Build.MODEL);
            cfg.append(String.format("setenv IV_PLAT_VER %s\n", openVpnEscape(versionString)));
        }
        ...
        for (String serverConfigLine : serverConfigLines) {
            if (serverConfigLine.contains("dev tun")) {
                ...
                // Add remote here TCT/UDP and Proto
                cfg.append("remote ").append(serverIp).append(" ").append(port).append(" ").append(protocol)
                        .append("\n");
                if (stunnelRoutingIp != null) {
                    cfg.append("route ").append(stunnelRoutingIp).append(" 255.255.255.255 net_gateway\n");
                }
            } else {
                if (!serverConfigLine.contains("persist-tun")) {
                    if (!serverConfigLine.contains("nobind")) {
                        if (!serverConfigLine.contains("remote-cert-tls")) {
                            if (serverConfigLine.contains("key-direction")) {
                                cfg.append(serverConfigLine).append("\n");
                                cfg.append("route-ipv6 ::/0\n");
                                cfg.append("route 0.0.0.0 0.0.0.0 vpn_gateway\n");
                                cfg.append("nobind\n");
                                cfg.append("remote-cert-tls server\n");
                                cfg.append("persist-tun\n");
                                cfg.append("preresolve\n");
    ...
    private String getConfigFileBlocking(Context context, String windServerConfig) {
        ...
        // if authentication is not username add Inline
        if (!isUsernameRequired(windServerConfig)) {
            cfg.append("ca [inline]\n");
            cfg.append("cert [inline]\n");
            cfg.append("key [inline]\n");
            cfg.append("tls-auth [inline] 1\n");
        }

        boolean removeNoBind = containsLPortAndNoBind(serverConfigLines);

        for (String serverConfigLine : serverConfigLines) {
            if (removeNoBind && serverConfigLine.contains("nobind")) {
                Log.i("vpn profile", "Removing nobind");
            } else {
                cfg.append(serverConfigLine).append("\n");
            }
        }
    ...
    private boolean containsLPortAndNoBind(String[] serverConfigLines) {
        boolean containsLPort = false;
        boolean containsNoBind = false;
        for (String serverConfigLine : serverConfigLines) {
            if (serverConfigLine.contains("lport")) {
                containsLPort = true;
            }
            if (serverConfigLine.contains("nobind")) {
                containsNoBind = true;
        ...
        return containsLPort && containsNoBind;
    }

    private boolean isUsernameRequired(String windServerConfig) {
        String[] serverConfigLines = windServerConfig.split(System.getProperty("line.separator"));
        for (String serverConfigLine : serverConfigLines) {
            if (serverConfigLine.contains("auth-user-pass")) {
                return true;
            }
        }
        return false;
